...
func main() {
    // Initialize SQLite Database (modernc.org/sqlite)
    if err := db.InitDB("savetune.db"); err != nil {
        log.Fatalf("Failed to initialize SQLite database: %v", err)
    }
    defer db.CloseDB()

    // Initialize Downloader Engine (goroutine manager)
    ...
    v1 := r.Group("/api/v1")
    {
        v1.GET("/health", func(c *gin.Context) {
            c.JSON(200, gin.H{"status": "ok", "authenticated": spotify.IsAuthenticated()})
        })
    ...
    // WebSocket route for real-time download progress
    r.GET("/ws/downloads", handlers.WebSocketDownloads)

    // Start Go server specifically on 127.0.0.1
    // The frontend will communicate over localhost HTTP.
    log.Println("Starting SaveTune Mobile Backend on 127.0.0.1:7799")
    if err := r.Run("127.0.0.1:7799"); err != nil {
...
void main() async {
  ...
  for (int i = 0; i < 10; i++) {
    try {
      ...
      if (res.statusCode == 200) { serverReady = true; break; }
    } catch (_) {
  ...
  final savedSpDc = await storage.read(key: 'sp_dc');
  final bool hasSpDc = savedSpDc != null && savedSpDc.trim().isNotEmpty;

  // If sp_dc exists, re-send it to Go backend (Go loses it on restart)
  if (hasSpDc && serverReady) {
    try {
      await Dio().post(
        'http://127.0.0.1:7799/api/v1/config/spdc',
        data: {'sp_dc': savedSpDc},
      );
    } catch (_) {}
  }
  ...
  runApp(
    ProviderScope(
      child: SaveTuneApp(initialRoute: hasSpDc ? '/search' : '/settings'),
    ),
  );
}

class SaveTuneApp extends StatelessWidget {
  final String initialRoute;
  const SaveTuneApp({super.key, required this.initialRoute});
...
class SaveTuneApi {
  ...
  Future<SearchResultModel> search(String query, {
    ...
    try {
      ...
      // We manually convert map to SearchResultModel. Note backend tracks wrapper.
...
"lib/features/search/widgets/track_card.dart": """import 'package:flutter/material.dart';
import '../../../../shared/models/track_model.dart';
import '../../../../app/theme.dart';
import '../../../../shared/api/client.dart';
...
"lib/shared/widgets/bottom_nav.dart": """import 'package:flutter/material.dart';
import 'package:go_router/go_router.dart';
import '../../app/theme.dart';
...
""",
"lib/features/player/player_screen.dart": """import 'package:flutter/material.dart';
...
"lib/features/lyrics/lyrics_screen.dart": """import 'package:flutter/material.dart';
